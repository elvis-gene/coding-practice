...
one of length 4 and one of length 3. The number 20 has binary representation 10100 and contains one binary gap of length 1. The number 15 has binary representation 1111 and 
...
def solution(N):
    ...
    for window_end in bin_str:

        if window_end == '0':
            if window_end not in char_frequency:
                char_frequency[window_end] = 1
            else:
                char_frequency[window_end] += 1
        
        else:
            if char_frequency:
                max_len = max(max_len, char_frequency['0']) 
                del char_frequency['0']
