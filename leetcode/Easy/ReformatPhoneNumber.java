...
class Solution {
    public String reformatNumber(String number) {
        ...
        while (number.length() > 4){
            output.append(number.substring(0, 3)).append("-");
            number = number.substring(3); // from three until the end of the string
        }
        
        if (number.length() == 4){
            output.append(number.substring(0, 2)).append("-");
            output.append(number.substring(2, 4));
        }
        else if(number.length() == 3){ 
            output.append(number.substring(0, 3));
        }
        else { // length of number == 2
            output.append(number.substring(0, 2));
        }
